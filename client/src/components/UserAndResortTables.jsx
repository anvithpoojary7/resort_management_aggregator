...
const UserAndResortTables = ({ initialTab = 'users' }) => { // Default to 'users' tab if not specified
  ...
  // Effect to set initial tab if the initialTab prop changes (e.g., navigating directly to /admin/resorts)
  useEffect(() => {
  ...
  useEffect(() => {
    const fetchUsers = async () => {
      try {
        ...
        setErrorUsers(null);
        // --- REPLACE WITH YOUR ACTUAL API CALL FOR USERS ---
    ...
    if (activeTab === 'users') { // Only fetch users if the 'users' tab is active
      fetchUsers();
    }
  }, [activeTab]); // Dependency: Re-fetch users when activeTab changes to 'users'
  ...
  useEffect(() => {
    const fetchResorts = async () => {
      try {
        ...
        // --- REPLACE WITH YOUR ACTUAL API CALL FOR RESORTS ---
        // Example: const response = await axios.get('/api/resorts');
        const response = await new Promise(resolve => setTimeout(() => {
          resolve({
            data: [
    ...
    if (activeTab === 'resorts') { // Only fetch resorts if the 'resorts' tab is active
      fetchResorts();
    }
  }, [activeTab]); // Dependency: Re-fetch resorts when activeTab changes to 'resorts'
      ...
      {activeTab === 'users' ? (
        ...
        <>
          ...
          {!loadingResorts && !errorResorts && (
            <ResortTable resorts={filteredData} />
          )}
        </>
      )}
    </section>
