...
const ModerationPage = () => {
  ...
  useEffect(() => {
    const fetchPendingResorts = async () => {
      try {
        // No need to get token from localStorage, browser sends httpOnly cookie automatically
        const res = await fetch(`${API_BASE_URL}/api/resorts?status=pending`, {
          method: 'GET',
          headers: {
            'Content-Type': 'application/json',
            // No 'Authorization' header needed for httpOnly cookies
          },
  ...
  const handleModerationAction = async (resortId, action) => {
    try {
      ...
      const res = await fetch(`${API_BASE_URL}/api/resorts/${resortId}/status`, {
        method: 'PATCH', // Corrected: Must be PATCH
        headers: {
          'Content-Type': 'application/json',
          // No 'Authorization' header needed for httpOnly cookies
        },
